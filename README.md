...
发布包可以解压到包含中文或空格的目录；WebView2 页面地址会按 Windows 文件路径校验，不会因为 URL 编码差异退回旧版原生面板。

输入框旁可快速切换规划模式、逐次审批和自动编辑。规划模式用于只读分析，逐次审批会在敏感操作前请求确认；自动编辑允许文件编辑，命令仍由 CLI 权限规则控制。切换只在当前请求结束后启用，会重启 CLI 并尝试续接当前会话；切换失败会保留原模式并显示原因。完整权限配置仍可从快捷菜单进入 Settings。

输入框上方始终保留解释、修复、优化、注释、诊断和学习六项常用操作，窄面板可横向滚动；“更多”菜单也提供相同操作。它们优先使用编辑器选区，没有选区则使用当前文件；普通聊天也会附带当前编辑内容（包括未保存内容，最多 24,000 字节）和最近的编译错误。每条用户消息都会将实际发送的上下文快照保存在本机会话中，并在消息下方以折叠详情显示，便于复盘 AI 回答所依据的代码。关闭“附加 IDE 上下文”后仍可查看摘要，但快照不会发送或保存。Copy answer 复制当前回答，Open code 将回答的第一个完整 Markdown 代码块打开为新编辑标签页，便于检查、修改和编译。Clear 清空当前运行期对话，Context 重新收集当前编辑器和编译上下文。Logs 控制后续诊断日志显示，API 重试和错误仍会显示；思考块不再混入正文。

输入框和回答区支持 Ctrl+C、Ctrl+V、Ctrl+A，以及右键菜单；输入框还支持剪切、撤销和 Shift+Insert 粘贴。在回答区粘贴会把文字放进输入框。流式回复到来时会保留已选中的文字，便于复制。Markdown 会渲染标题、列表、引用、粗体、行内代码、分隔线和表格；围栏代码块使用等宽字体，并对常见 C/C++ 关键字、字符串和注释着色。代码块工具栏可切换软换行、行号与折叠；行号只用于阅读，复制、打开和插入仍保持原始代码，流式更新时也会保留显示状态。工具调用在发生时插入对话流，每个调用独立折叠；结果更新原位置，前后文字顺序不变。点击工具标题展开参数与结果，支持复制；历史恢复保留这些块的顺序。消息区滚轮保持当前位置，回到底部按钮可恢复最新消息。
...
Windows 构建中的 AgentMainSmoke 使用本地模拟 CLI 检查真实的 `--resume` 参数、会话切换和重启恢复，不使用 API Key 或网络；AgentUISmoke 覆盖工具折叠、正文隔离、逐条 IDE 上下文快照保存与重载、Markdown 显示和输入草稿。
