...
面板顶部选择 Explain code、Fix code、Improve code、Add comments 或 Diagnose build errors，再点 Run。优先使用编辑器选区，没有选区则使用当前文件；普通聊天也会附带当前编辑内容（包括未保存内容，最多 24,000 字节）和最近的编译错误。Copy answer 复制当前回答，Open code 将回答的第一个完整 Markdown 代码块打开为新编辑标签页，便于检查、修改和编译。Clear 清空当前运行期对话，Context 重新收集当前编辑器和编译上下文。Logs 控制后续诊断日志显示，API 重试和错误仍会显示；思考块不再混入正文。

输入框和回答区支持 Ctrl+C、Ctrl+V、Ctrl+A，以及右键菜单；输入框还支持剪切、撤销和 Shift+Insert 粘贴。在回答区粘贴会把文字放进输入框。流式回复到来时会保留已选中的文字，便于复制。Markdown 会渲染标题、列表、引用、粗体、行内代码、分隔线和表格；围栏代码块使用等宽字体，并对常见 C/C++ 关键字、字符串和注释着色。工具调用在发生时插入对话流，每个调用独立折叠；结果更新原位置，前后文字顺序不变。点击工具标题展开参数与结果，支持复制；历史恢复保留这些块的顺序。消息区滚轮保持当前位置，回到底部按钮可恢复最新消息。
