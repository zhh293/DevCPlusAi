...
var
  InCComment:boolean;
  function FirstLineChar(const Line: AnsiString): AnsiChar;
  begin
    if Length(Line) > 0 then
      Result := AnsiChar(TrimLeft(Line)[1]) // preprocessor markers are ASCII
    else
      Result := AnsiChar(#0);
  end;
begin
...
procedure TCppPreprocessor.SkipToEndOfPreprocessor;
  function LastLineChar(const Line: AnsiString): AnsiChar;
  begin
    if Length(Line) > 0 then
...
var
  ...
  var
    ...
    var
      ...
      UnicodeArgs, UnicodeArgValues: string;
      I: integer;
    begin
      ...
      try
        UnicodeArgs := string(FunctionDefine^.Args);
        UnicodeArgValues := string(ArgValueString);
        ExtractStrings([',', '(', ')'], [], PChar(UnicodeArgs), ArgNames);
        ExtractStrings([',', '(', ')'], [], PChar(UnicodeArgValues), ArgValues); // extract from Line string
