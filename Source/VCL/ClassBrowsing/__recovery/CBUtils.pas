...
type
  ...
  { Compatible replacement for the linked-list TStringHash used by Delphi 7. }
  TDevStringHash = class
  private
  ...
  public
    ...
    procedure Add(const Key: string; const Value: Integer);
    procedure Clear;
...
constructor TDevStringHash.Create(Size: Cardinal);
begin
  inherited Create;
  if Size = 0 then
    Size := 256;
...
destructor TDevStringHash.Destroy;
begin
  Clear;
  inherited Destroy;
end;

function TDevStringHash.HashOf(const Key: string): Cardinal;
var
  P: PChar;
begin
  Result := 0;
...
procedure TDevStringHash.Add(const Key: string; const Value: Integer);
var
  Item: PHashItem;
  InsertAt: PPHashItem;
  Bucket: Cardinal;
begin
  Bucket := HashOf(Key) mod Cardinal(Length(FBuckets));
  InsertAt := @FBuckets[Bucket];
  while (InsertAt^ <> nil) and (InsertAt^^.Key <> Key) do
...
procedure TDevStringHash.Clear;
var
  I: Integer;
  Item, NextItem: PHashItem;
begin
...
var
  ...
  Previous: PPHashItem;
begin
  Previous := FindItem(Key);
  Item := Previous^;
  while (Item <> nil) and (Item^.Key = Key) and (Item^.Value <> Value) do begin
    Previous := @Item^.Next;
    Item := Previous^;
  end;
  if (Item <> nil) and (Item^.Key = Key) then begin
    Previous^ := Item^.Next;
    Dispose(Item);
  end;
end;

function TDevStringHash.ValueOf(const Key: string): Integer;
var
  Item: PPHashItem;
begin
  Item := FindItem(Key);
