...
begin
  ...
  if RedirectInput then begin
    ...
    InputWrite := CreateNamedPipeW(
          PWideChar(Pipename),             // pipe name
  ...
  CommandLine := '"' + fFile + '" ' + params;
  UniqueString(CommandLine);
  if CreateProcessW(nil, PWideChar(CommandLine), nil, nil, True,
    flags, nil,
    PWideChar(fPath), StartupInfo, ProcessInfo) then begin
...
begin
  FileHandle := CreateFileW(PWideChar(InputFile),GENERIC_READ, FILE_SHARE_READ,
