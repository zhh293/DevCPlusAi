...
begin
  with devData do begin
    ...
    if not MultiLineTab then begin
      if cboTabsTop.ItemIndex in [2, 3] then begin
        ErrorMessage := string(Lang[ID_ENV_MULTILINETABERROR]);
        ErrorCaption := string(Lang[ID_ERROR]);
        MessageBoxW(application.handle, PWideChar(ErrorMessage), PWideChar(ErrorCaption), MB_OK);
  ...
  try
    ...
    ErrorMessage := string(Lang[ID_ENV_UACERROR]);
    ErrorCaption := string(Lang[ID_ERROR]);
    MessageBoxW(application.handle, PWideChar(ErrorMessage), PWideChar(ErrorCaption), MB_OK);
