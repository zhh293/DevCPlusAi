...
uses
  ...
  devFileBrowser,
  AgentPanel, AgentProcess, AgentReader, AgentProtocol, System.Actions;
...
//    procedure actCompileRunUpdate(Sender: TObject);
  ...
  private
    ...
    fAgentReader : TAgentReader;
    fAgentPanelFrame : TAgentPanelFrame;
    fAgentPanel : TPanel;
    fAgentSplitter : TSplitter;
    fActAIAssistant : TAction;
    fActAISettings : TAction;
    fActAIFocus : TAction;
    fAgentSelectionActions : array[0..3] of TAction;
    fAgentPendingFilePath : String;
...
uses
  ...
  devParser, AgentSetupFrm, AgentConfig;
...
begin
  ...
  // AI Agent: persist panel width and tear down the process/reader
  if Assigned(fAgentPanel) and Assigned(devAgentConfig) then
    devAgentConfig.PanelWidth := fAgentPanel.Width;
  StopAgent;
end;
...
var
  I, Count: integer;
  FileNameBuffer: array[0..4095] of char;
  MessageHandle: THandle;
  FileList: TStringList;
  DropPoint, ScreenPoint, PanelOrigin: TPoint;
  PanelRect: TRect;
  DropOnAgentPanel: Boolean;
begin
  try
    // Get drop information
    MessageHandle := THandle(msg.wParam);
    Count := DragQueryFile(MessageHandle, $FFFFFFFF, nil, 0);
    DropOnAgentPanel := False;
    if Assigned(fAgentPanelFrame) and DragQueryPoint(MessageHandle, DropPoint) then begin
      ScreenPoint := ClientToScreen(DropPoint);
      PanelRect := fAgentPanelFrame.ClientRect;
      PanelOrigin := fAgentPanelFrame.ClientToScreen(Point(0, 0));
      OffsetRect(PanelRect, PanelOrigin.X, PanelOrigin.Y);
      DropOnAgentPanel := PtInRect(PanelRect, ScreenPoint) and
        fAgentPanelFrame.Visible and fAgentPanel.Visible;
    end;
    ...
    try
      ...
      if DropOnAgentPanel then begin
        fAgentPanelFrame.AddDroppedFiles(FileList);
...
//  MessageControl.Height := devData.OutputHeight;
  ...
  SetupAgentPanel;
  if Assigned(devAgentConfig) and devAgentConfig.Enabled
    and (Trim(devAgentConfig.ApiKey) <> '') then
    StartAgent;
...
begin
  ...
  if Assigned(devAgentConfig) and (Trim(devAgentConfig.ApiKey) = '') then begin
    with TAgentSetupForm.Create(Self) do begin
      if ShowModal = mrOk then
        StartAgent;
      // The form frees itself in OnClose (caFree).
    end;
  end;
end;
...
procedure TMainForm.SetupAgentPanel;
var
  w, I: Integer;
  mi: TMenuItem;
begin
  ...
  fAgentSessionId := '';
  fAgentWorkDir := '';
  fAgentResumeAttempted := False;
  fAgentIgnoreSavedSession := False;
  fAgentRestartAttempts := 0;
  fAgentExpectedStop := False;
  fAgentPendingFiles := TStringList.Create;
  fAgentRestartTimer := TTimer.Create(Self);
  ...
  fAgentSplitter.Parent := Self;
  fAgentSplitter.Align := alRight;
  fAgentSplitter.Width := 4;

  // Right-hand container panel
  fAgentPanel := TPanel.Create(Self);
  ...
  for I := 3 downto 0 do begin
    ...
    fAgentSelectionActions[I].ActionList := ActionList;
  ...
  if Assigned(devAgentConfig) and devAgentConfig.Enabled then begin
    fAgentPanel.Visible := True;
    fAgentSplitter.Visible := True;
  end else begin
    fAgentPanel.Visible := False;
    fAgentSplitter.Visible := False;
  end;
end;

procedure TMainForm.ActAIAssistantExecute(Sender: TObject);
begin
  if not Assigned(fAgentPanel) then
    Exit;
  fAgentPanel.Visible := not fAgentPanel.Visible;
  fAgentSplitter.Visible := fAgentPanel.Visible;
end;

procedure TMainForm.AgentSettingsExecute(Sender: TObject);
begin
  with TAgentSetupForm.Create(Self) do begin
    if ShowModal = mrOk then
      RestartAgentForCurrentProject;
...
begin
  ...
  if Assigned(fAgentPanelFrame) then
    fAgentPanelFrame.SendPrompt(Prompt);
end;

function TMainForm.AgentSessionFileName(const WorkDir: String): String;
var
  BaseDir, Key, HashText: String;
  I: Integer;
  Hash: Cardinal;
begin
  BaseDir := IncludeTrailingPathDelimiter(devDirs.Config) + 'AgentSessions';
  if not DirectoryExists(BaseDir) then
    ForceDirectories(BaseDir);

  if Assigned(fProject) and (fProject.FileName <> '') then
    Key := fProject.FileName
  else
    Key := WorkDir;
  if Key = '' then
    Key := 'standalone';

  // Keep the readable portion for diagnostics, but add a stable hash so two
  // projects with the same truncated suffix cannot share a session file.
  Hash := 2166136261;
  for I := 1 to Length(Key) do
    Hash := (Hash xor Ord(Key[I])) * 16777619;
  HashText := IntToHex(Hash, 8);
  for I := 1 to Length(Key) do
...
begin
  ...
  try
    ...
    Lines := Trim(List.Text);
    if (Lines = '') or (Length(Lines) > 128) then
      Exit;
    for I := 1 to Length(Lines) do
      if not (Lines[I] in ['a'..'z', 'A'..'Z', '0'..'9', '-', '_']) then
        Exit;
    Value := Lines;
    Result := Value;
  finally
...
procedure TMainForm.SaveAgentSession(const FileName, SessionId: String);
var
  List: TStringList;
  TempFile: String;
  I: Integer;
begin
  ...
  for I := 1 to Length(SessionId) do
    if not (SessionId[I] in ['a'..'z', 'A'..'Z', '0'..'9', '-', '_']) then
      Exit;
  List := TStringList.Create;
  try
    List.Text := SessionId;
    ...
    try
      ...
      // not prevent the IDE from continuing with a new Claude session.
    end;
  finally
    List.Free;
  end;
...
procedure TMainForm.RefreshAgentFile(const FileName: String);
var
  Path: String;
  E: TEditor;
begin
  if (FileName = '') or not Assigned(fAgentPanelFrame) then
    Exit;

  Path := StringReplace(FileName, '/', '\', [rfReplaceAll]);
  if ((Length(Path) < 2) or (Path[2] <> ':')) and
     not StartsText('\\', Path) then begin
    if Assigned(fAgentProcess) and (fAgentProcess.WorkDir <> '') then
      Path := IncludeTrailingPathDelimiter(fAgentProcess.WorkDir) + Path
    else if fAgentWorkDir <> '' then
      Path := IncludeTrailingPathDelimiter(fAgentWorkDir) + Path
    else
      Path := IncludeTrailingPathDelimiter(devDirs.Exec) + Path;
  end;
  Path := ExpandFileName(Path);
  if not FileExists(Path) then
    Exit;

  E := fEditorList.GetEditorFromFileName(Path);
  if not Assigned(E) then
    Exit;
  if E.Text.Modified then begin
...
var
  workDir, sessionFile, sessionId: String;
begin
  if not Assigned(fAgentPanelFrame) then
    Exit;
  ...
  if (fAgentSessionFile <> '') and
     not SameText(fAgentSessionFile, sessionFile) then begin
  ...
  if Assigned(devAgentConfig) then
    fAgentProcess.CliPath := devAgentConfig.CliPath
  else
    fAgentProcess.CliPath := '';
  fAgentProcess.ResumeSessionId := sessionId;

  // Stream events carry state across several JSONL lines. Never reuse that
  // state when a new child process/session starts.
  ResetProtocolState;
  if Assigned(fAgentPendingFiles) then
    fAgentPendingFiles.Clear;
  fAgentPendingFilePath := '';

  fAgentPanelFrame.AgentProcess := fAgentProcess;
...
procedure TMainForm.RestartAgentForCurrentProject;
begin
  if fQuitting then
    Exit;
  StopAgent;
  ...
  if Assigned(fAgentRestartTimer) then
    fAgentRestartTimer.Interval := 1000;
  if Assigned(devAgentConfig) and devAgentConfig.Enabled and
     (Trim(devAgentConfig.ApiKey) <> '') then
    StartAgent
  else if Assigned(fAgentPanelFrame) then
...
begin
  ...
  if Assigned(fAgentReader) then begin
    fAgentReader.Terminate;
  ...
  if Assigned(fAgentPanelFrame) then
    fAgentPanelFrame.AgentProcess := nil;
  ResetProtocolState;
  if Assigned(fAgentPendingFiles) then
    fAgentPendingFiles.Clear;
  fAgentPendingFilePath := '';
...
procedure TMainForm.AgentLineReady(const Line: String);
var
...
begin
  ...
  try
    for I := 0 to Length(Events) - 1 do begin
      ...
      if ev.EventType = aetToolUse then begin
        if ev.FilePath <> '' then begin
          fAgentPendingFilePath := ev.FilePath;
          if (ev.ToolId <> '') and Assigned(fAgentPendingFiles) then
            fAgentPendingFiles.Values[ev.ToolId] := ev.FilePath;
        end;
      end else if ev.EventType = aetToolResult then begin
        Path := ev.FilePath;
        if (Path = '') and (ev.ToolId <> '') and Assigned(fAgentPendingFiles) then begin
          MapIndex := fAgentPendingFiles.IndexOfName(ev.ToolId);
          if MapIndex >= 0 then begin
            Path := fAgentPendingFiles.ValueFromIndex[MapIndex];
            fAgentPendingFiles.Delete(MapIndex);
          end;
        end;
        if Path = '' then
          Path := fAgentPendingFilePath;
        RefreshAgentFile(Path);
        if SameText(Path, fAgentPendingFilePath) then
          fAgentPendingFilePath := '';
      end else if ev.EventType = aetResult then begin
        // Some CLI versions omit a separate tool_result. Refresh every
        // still-pending path when the turn ends instead of only the last one.
        if Assigned(fAgentPendingFiles) then begin
          for MapIndex := fAgentPendingFiles.Count - 1 downto 0 do begin
            Path := fAgentPendingFiles.ValueFromIndex[MapIndex];
            RefreshAgentFile(Path);
          end;
          fAgentPendingFiles.Clear;
        end;
        if fAgentPendingFilePath <> '' then begin
          RefreshAgentFile(fAgentPendingFilePath);
          fAgentPendingFilePath := '';
        end;
      end;
      if Assigned(fAgentPanelFrame) then
...
begin
  ...
  if Assigned(fAgentPanelFrame) then
    fAgentPanelFrame.SetStatus(asDisconnected);
  if fAgentRestartAttempts < 3 then begin
    Inc(fAgentRestartAttempts);
    if Assigned(fAgentRestartTimer) then begin
...
begin
  if Assigned(fAgentRestartTimer) then
    fAgentRestartTimer.Enabled := False;
  if fQuitting or fAgentExpectedStop then
    Exit;
  StopAgent;
  StartAgent;
  if not (Assigned(fAgentProcess) and fAgentProcess.IsRunning) then begin
    if fAgentRestartAttempts < 3 then
      Inc(fAgentRestartAttempts);
    if (fAgentRestartAttempts < 3) and Assigned(fAgentRestartTimer) then begin
      fAgentRestartTimer.Interval := 1000 * fAgentRestartAttempts;
      fAgentRestartTimer.Enabled := True
    end else if Assigned(fAgentPanelFrame) then
