...
begin
  // WM_COPYDATA is synchronous, so Data's buffer remains valid until SendMessage returns.
  DataStruct.dwData := SENDDATAID;
  DataStruct.cbData := SizeOf(AnsiChar) * (Length(Data) + 1);
  DataStruct.lpData := PAnsiChar(Data);
