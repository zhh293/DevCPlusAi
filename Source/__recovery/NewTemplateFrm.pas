...
begin
  ...
  with tmpIni do try
    ...
    if IconFiles[0] <> '' then begin
      SourceFile := IconFiles[0];
      DestFile := devDirs.Templates + cmbName.Text + '.ico';
      CopyFileW(PWideChar(SourceFile), PWideChar(DestFile), False);
    ...
    for I := 0 to lstFiles.Items.Count - 1 do
      if lstFiles.Checked[I] then begin
        ...
        SourceFile := TempProject.Units[I].FileName;
        DestFile := devDirs.Templates + S;
        CopyFileW(PWideChar(SourceFile), PWideChar(DestFile), False);
        Inc(C);
      end;
    ...
    if IconFiles[1] <> '' then begin
      SourceFile := IconFiles[1];
      DestFile := devDirs.Templates + cmbName.Text + '.project.ico';
      CopyFileW(PWideChar(SourceFile), PWideChar(DestFile), False);
